...
import { usePublicTourPackageStore } from '@/stores/tourpackage';
import type { PackageImage } from '@/types/tourpackage';
import { usePublicDestinationStore } from '@/stores/destinations';
import { usePublicTourScheduleStore } from '@/stores/tourschedule';
...
const tourPackageStore = usePublicTourPackageStore();
const destinationStore = usePublicDestinationStore();
const tourScheduleStore = usePublicTourScheduleStore();

const { tourPackages, loading, error } = storeToRefs(tourPackageStore);
const {destinations} = storeToRefs(destinationStore)
const {tourSchedules} = storeToRefs(tourScheduleStore)

onMounted(() => {
  tourPackageStore.fetchTourPackages();
  destinationStore.fetchDestinations();
  tourScheduleStore.fetchTourSchedules();
});
...
const headerClass = computed(() => {
    return {
        'fixed top-0 inset-x-0 z-9999 transition-all duration-300':true,
...
const ABOUT_US:AboutUs[] = [
  { icon: Mountain, title: "Pemandu Lokal", desc: "Bersertifikat & berpengalaman" },
  { icon: Star, title: "Rating 4.8", desc: "Dari ulasan terverifikasi" },
  { icon: Users, title: "Grup Kecil", desc: "Maks. 12 peserta" },
  { icon: TreePine, title: "Ramah Lingkungan", desc: "Leave No Trace" },
]

const MEDSOS = [Instagram, Facebook]
...
<template>
      ...
      <section class="relative bg-white overflow-hidden pt-16">
        ...
        <ActivityCard :tourPackages="tourPackages"/>
                ...
                >
                  <div class="flex">
                    ...
                    <div class="w-28 lg:w-36 flex-shrink-0 bg-gray-100 overflow-hidden">
                      <img
                        :src="v.tour_package.image_url"
                        :alt="v.tour_package.title"
                    ...
                    <div class="flex-1 p-4 lg:p-5">
                      <div class="flex items-start justify-between gap-2 mb-2">
                        <div>
                          <h3 class="text-base lg:text-lg font-semibold text-gray-900 leading-snug">
                            {{ v.tour_package.title }}
                          </h3>
                          <div class="flex items-center gap-1 text-gray-400 text-xs mt-0.5">
                            <MapPin class="w-3 h-3" />
                            {{ v.tour_package.location }}
                          </div>
                        </div>
                      ...
                      <div class="mb-3">
                        <div class="flex justify-between text-xs mb-1">
                          <span class="text-gray-500 flex items-center gap-1">
                            <Users class="w-3 h-3" />
                            <span v-if="v.is_full == 1" class="text-red-500 font-medium">Penuh</span> 
                            <span v-else>{{v.available_quota}} slot tersisa</span>
                          </span>
                          <span class="text-gray-400">{{v.joined}}/{{ v.quota }}</span>
                        </div>
                      ...
                      <div class="flex items-center justify-between">
                        <div>
                          <span class="font-['Fraunces',serif] font-bold text-gray-900">{{ v.tour_package.price }}</span>
                          <span class="text-gray-400 text-xs ml-1">/orang</span>
                        </div>
                        <button
                          :class="tripButtonClass(v.is_full)"
                        >
                        {{ v.is_full ? "Waitlist": "Daftar"}}
                        </button>
                      </div>
                    </div>
                  </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20 items-center">
            ...
            <div class="order-1 lg:order-2">
              ...
              <div class="grid grid-cols-2 gap-3 mb-8">
                  <div v-for="v in ABOUT_US" class="p-4 rounded-xl bg-gray-50 border border-gray-100">
                    <div class="mb-2">
                      <component
                          :is="v.icon"
                    ...
                    <div class="text-gray-900 font-semibold text-sm">{{ v.title }}</div>
                    <div class="text-gray-400 text-xs mt-0.5">{{ v.desc }}</div>
                  </div>
              </div>
